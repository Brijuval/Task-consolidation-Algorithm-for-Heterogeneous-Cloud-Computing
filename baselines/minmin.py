...
class MinMinScheduler:
    ...
    def run_epoch(self, epoch, sim_time, tasks):
        ...
        for task in tasks:
            best_vm = min(self.vms, key=lambda v: task.exec_time_on(v.mips) if v.mips > 0 else 9999)
            best_vm.current_load = min(1.0, best_vm.current_load + 0.05)
            exec_time = task.exec_time_on(best_vm.mips) if best_vm.mips > 0 else 9999
